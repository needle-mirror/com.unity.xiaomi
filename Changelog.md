## [??.??.??] - ????-??-??
- XiaomiMiPay - add Xiaomi MiPay app store purchasing. Note, mandatory app store login functionality is provided by the new Unity Channel SDK and service, also included in this release of Unity IAP. To use, first enable the app store through the Window > Unity IAP > Android menu before building an APK. See below for more on Unity Channel SDK initialization and login integration. See a sample usage of this app store in `IAPDemo.cs`.
...
- UnityChannel - includes a new Unity service for unified login and payment management with Unity-supported Chinese app stores. Login is required to access Xiaomi MiPay by Unity IAP. For app store login functionality, call client API `UnityEngine.Store.StoreService.Initialize` then `UnityEngine.Store.StoreService.Login`, at app launch, before Unity IAP initialization. For optional extended post-purchase transaction actions, such as remote receipt validation, use the `IUnityChannelExtensions` instance collected from an app's Unity IAP `IStoreListener.OnInitialized` callback with `var ucExt = extensions.GetExtension<IUnityChannelExtensions>();`. Several related helper types are also included, e.g. to facilitate working with completed transactions. Note, to configure builds for mandatory Chinese Government audit and app store testing set the `AppInfo.debug` to `true` field when calling `StoreService.Initialize`. See sample `UnityChannel` integration in `IAPDemo.cs`. Note, while the IAP Catalog window is supported by UnityChannel, Codeless IAP's "IAP Button" is not currently supported. New API:
...
{
	// Unpack Unity IAP's unified receipt string.
	...
	// Unpack the Unity Channel receipt from a unified receipt string.
	// var unifiedReceipt = JsonUtility.FromJson<UnifiedReceipt>(purchEvtArg.purchasedProduct.receipt)
	...
	public enum UnityChannelTestMode
	{
		Production,
		Debug,
	}

	// NOTE: For future use. Currently only 'Production' supported by backend.
	// Sets flag for test-mode. To use, first set the current store to one 
	// supported by UnityChannel such as AndroidStore.XiaomiMiPay.
	public static void SetUnityChannelTestMode(UnityChannelTestMode testMode)

	public interface IUnityChannelExtensions : IStoreExtension
	{
	    /// <summary>
	    /// Collects purchase status for a prior purchase. Use to review purchase history, especially in
	    /// game-interruption or network timeout situations.
	    /// </summary>
	    /// <param name="gameOrderId">Original client-generated transaction identifier.</param>
	    /// <param name="callback">Callback returning bool success, string 'confirmedGameOrderId', and string 'errorMessage'.</param>
	    void ConfirmPurchase(string gameOrderId, Action<bool, string, string> callback);

		/// <summary>
		/// Validates the receipt of a given gameOrderId.
		/// </summary>
		/// <param name="gameOrderId">Original client-generated transaction identifier.</param>
		/// <param name="callback">Callback returning bool success, string 'signData', and string 'signature'.</param>
